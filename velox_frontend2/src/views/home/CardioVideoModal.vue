...
import CardioVideoForm from "./CardioVideoForm";
...
export default {
  ...
  methods: {
    ...
    validate() {
      ...
      // if (!this.videoData.cardio_type) errors.push("Please select cardio type");
    ...
    async onModalSave() {
      ...
      this.measureData = {
        ...
        // cardio_quality: this.videoData.video_qc,
        // cardio_type: this.videoData.cardio_type
      };

      try {
        let response = await commonServices.getSignedUrl({
        ...
        this.measureData.cardio_video = {};

        this.measureData.cardio_video.path = response.data.gcs_path;
        this.measureData.cardio_video.bucket = response.data.gcs_bucket;
        this.measureData.cardio_video.filename = response.data.gcs_filename;

        this.videoData.file.url = response.data.url;
    ...
    async onVideoUploadCompleted(e) {
      ...
      try {
        console.log("this.measureData", this.measureData);
        console.log("this.measurementsData", this.measurementsData);

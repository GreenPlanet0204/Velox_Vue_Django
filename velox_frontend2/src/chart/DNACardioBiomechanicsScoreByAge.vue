  ...
  <div class="row">
    ...
    <div class="col-lg-10">
      <h5 class="text-center">
        DNA + Cardio + Biomechanics Score by Age
        <span
          ><b
            >{{ sum ? sum + " samples" : "" }} - ({{
              allelite +
...
export default {
  ...
  async mounted() {
    ...
    const chartData = this.DNACardioBiomechanicsScoreByAge(
      this.result?.data?.dna_cardio_biomechanics?.age
    );
    window.bar13 = new Chart(ctx, chartData);
    this.type = "all";
    this.chart_type();
    this.temp1 = this.result?.data?.dna_cardio_biomechanics?.age.map(
      item => item.counts
    );
  ...
  methods: {
    async chart_type(event) {
      ...
      const ctx = document.getElementById(
        "dna_cardio_biomechanics_score_by_age"
      );
      if (window.bar13 != undefined) {
        window.bar13.destroy();
      }

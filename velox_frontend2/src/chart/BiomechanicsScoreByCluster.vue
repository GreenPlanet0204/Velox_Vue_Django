...
export default {
  ...
  async mounted() {
    ...
    const chartData = this.BiomechanicsScoreByCluster(
      this.result?.data?.biomechanics?.cluster
    );
    window.bar7 = new Chart(ctx, chartData);
    this.type = "all";
    this.chart_type();
    this.temp1 = this.result?.data?.biomechanics?.cluster.map(
      item => item.counts
    );

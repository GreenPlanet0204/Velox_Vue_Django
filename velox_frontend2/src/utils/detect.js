...
export const detectImage = async (
  ...
  for (let idx = 0; idx < selected.dims[1]; idx++) {
    ...
    console.log("bounding", box[2], box[3]);
    if (box[2] * 0.8 > box[3]) {
      if (box[1] - 0.4 * box[2] > 0) {
        [x, y, w, h] = [
          (box[0] - 0.5 * box[2]) * xRatio, // upscale left
          (box[1] - 0.4 * box[2]) * yRatio, // upscale top
          box[2] * xRatio, // upscale width
          box[2] * 0.8 * yRatio, // upscale height
        ]; // keep boxes in maxSize range
      }
    } else {
      if (box[0] - 0.625 * box[3] > 0) {
        [x, y, w, h] = [
          (box[0] - 0.625 * box[3]) * xRatio, // upscale left
          (box[1] - 0.5 * box[3]) * yRatio, // upscale top
          box[3] * 1.25 * xRatio, // upscale width
          box[3] * yRatio, // upscale height
        ]; // keep boxes in maxSize range
      }
    }
